...
const InterceptorContext = {
    ...
    init: function ({setting = "", initCodeInput = false, propertiesHighlighting = false}) {
    ...
    showActionAlert: function ({containerId, message}) {
    ...
    setNoHooksNotificationVisible: function (visible) {
            ...
            if (xhr.readyState === 4) {
                if (xhr.status === 200 || xhr.status === 204) {
                    ...
                    if (onError === undefined) {
                        try {
                            InterceptorContext.showActionAlert({containerId: 'action-error', message: xhr.responseText});
                        } catch {
    ...
    readAndFillRevisions: function ({setting, configurationName = DEFAULT_CONFIGURATION, revertToRevisionCallback}) {
        ...
        this.callAsync({
            method: 'GET',
            url: `/polarion/interceptor/rest/internal/settings/${currentSetting}/revisions`,
            ...
            onOk: (responseText) => {
                ...
                for (const revision of revisions) {
                    const button = InterceptorContext.createRevertToRevisionButton();
                    button.addEventListener("click", function () {
                        InterceptorContext.hideActionAlerts();
                        InterceptorContext.callAsync({
                            method: 'GET',
                            url: `/polarion/interceptor/rest/internal/settings/${currentSetting}?revision=${revision.name}`,
                            contentType: 'application/json',
                            onOk: (responseText) => {
                                if (revertToRevisionCallback !== undefined) {
                                    revertToRevisionCallback(responseText);
                                }
                                InterceptorContext.showRevertedToRevisionAlert(revision);
                            }
                        });
                    });

                    InterceptorContext.createRevisionRow(tableBody, revision, button);
                }
            ...
            onError: () => InterceptorContext.setRevisionsLoadingErrorNotificationVisible(true)
        });
    },

    createRevisionRow: function (tableBody, revision, revertToRevisionButton) {
        const row = tableBody.insertRow();
        row.insertCell().appendChild(document.createTextNode(InterceptorContext.insertRevisionSpaces(revision.name)));
