...
const PROJECTS = {
  data: [{ id: 'elibrary', attributes: { name: 'E-Library' } }],
};
...
const aboutRoutes = () => [
  ...
  },
  {
    ...
    json: { properties: [], obsoleteProperties: [] },
  },
  { method: 'GET', match: /\/configuration-status/, json: [] },
  {
    method: 'GET',
    match: /\/readme$/,
    respond: () => new Response('<h1>Readme</h1>', { status: 200 }),
  },
];
...
afterEach(() => {
  ...
  document.cookie = 'interceptor-manager-dev-scope=; path=/; max-age=0';
});
...
describe('App router', () => {
  it('renders the Landing stub with every feature linked when no feature is selected', async () => {
    ...
    expect(links).toEqual(['?feature=about', '?feature=settings']);
  });
  ...
  it('renders the About page for ?feature=about', async () => {
    ...
    expect(document.body.textContent).toContain('Interceptor Manager');
